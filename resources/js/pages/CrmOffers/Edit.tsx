...
// Checkbox is not used for CrmOffer-specific fields, so can be removed if not generally needed
...
interface Props {
    crmOffer: CrmOffer & { items: OfferItemType[] }; // Changed prop name and type
...
interface CrmOfferFormData {
    ...
    offer_type: CrmOffer['offer_type'];
    notes?: CrmOffer['notes'];
    company_id?: CrmOffer['company_id'] | string | null; // string for select value
    contact_id?: CrmOffer['contact_id'] | string | null;
    assigned_user_id?: CrmOffer['assigned_user_id'] | string | null;
    items: OfferItemType[];
}

const createEmptyOfferItem = (): OfferItemType => ({
    service_name: '', description: '', quantity: '1', unit_price: '0',
    discount_percentage: null, tax_rate: null, total_price: 0, product_id: null,
});

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Dashboard', href: route('dashboard') },
    { title: 'CRM Offers', href: route('crm.offers.index') }, // Updated route
    { title: 'Edit CRM Offer' } // Updated title
];

const CrmOffersEdit: React.FC<Props> = ({
    crmOffer, errors: serverErrors, flash, companies, contacts: propContacts,
    users, statuses, offerTypes, products = [], defaultCurrency = 'SAR'
}) => { // Renamed component and prop
    const allContacts = Array.isArray(propContacts) ? propContacts : [];

    // Helper to format date strings for type="date" input (YYYY-MM-DD)
    ...
    const { data, setData, post, processing, errors, clearErrors, reset, transform } = useForm<CrmOfferFormData>({
        ...
        description: crmOffer.description || '',
        description_ar: crmOffer.description_ar || '',
        status: crmOffer.status || statuses[0],
        value: crmOffer.value ?? null,
        currency: crmOffer.currency || defaultCurrency,
        probability: crmOffer.probability ?? null,
        expected_close_date: formatDateForInput(crmOffer.expected_close_date),
        actual_close_date: formatDateForInput(crmOffer.actual_close_date),
        offer_type: crmOffer.offer_type || offerTypes[0],
        notes: crmOffer.notes || '',
        company_id: crmOffer.company_id?.toString() || null,
        contact_id: crmOffer.contact_id?.toString() || null,
        assigned_user_id: crmOffer.assigned_user_id?.toString() || null,
        items: crmOffer.items.map(item => ({
            ...item,
            quantity: item.quantity?.toString() || '1',
            unit_price: item.unit_price?.toString() || '0',
            discount_percentage: item.discount_percentage?.toString() ?? null,
            tax_rate: item.tax_rate?.toString() ?? null,
    ...
    useEffect(() => {
        ...
        if (data.company_id) {
            ...
            // Optionally, clear contact_id if the current one is no longer valid for the selected company
            const currentContactIsValid = newFiltered.some(c => c.id === Number(data.contact_id));
            if (data.contact_id !== null && !currentContactIsValid) {
                setData('contact_id', null);
            }
        } else {
            setFilteredContacts(currentAllContacts);
            // If no company is selected, clear contact_id
    ...
    const handleSelectChange = (name: keyof CrmOfferFormData, value: string | number | null) => {
        ...
        if (value === '') { // Treat empty string from select as null
            processedValue = null;
        }
    ...
    // No handleCheckboxChange needed as 'is_active' is not present
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        post(route('crm.offers.update', { offer: crmOffer.id }), {
            onSuccess: () => {
                toast.success('CRM Offer updated successfully!');
            },
            onError: (formErrors) => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit CRM Offer - ${crmOffer.title}`} /> {/* Updated title */}

            <div className="py-12">
                <Card className="max-w-4xl mx-auto sm:px-6 lg:px-8">
                    <CardHeader>
                        <CardTitle>Edit CRM Offer: {crmOffer.title}</CardTitle> {/* Updated title */}
                        <CardDescription>Update the details and line items for this CRM offer.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                {/* actual_close_date can also be added if needed on the form */}
                                {/* <div>
                                    <Label htmlFor="actual_close_date">Actual Close Date</Label>
                                    <Input id="actual_close_date" name="actual_close_date" type="date" value={data.actual_close_date || ''} onChange={handleInputChange} />
                                    {errors.actual_close_date && <p className="text-sm text-red-600 mt-1">{errors.actual_close_date}</p>}
                                </div> */}
                            </div>
                                    ...
                                    <div key={index} className="p-4 border rounded-md mb-4 space-y-3 bg-slate-50 dark:bg-slate-800">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                            ...
                                            <div className="md:col-span-2">
                                                ...
                                                <Select
                                                    value={item.product_id?.toString() || '_custom'} // Default to _custom if no product_id
                                            ...
                                            <div>
                                                ...
                                                <Input
                                                    ...
                                                    required={!item.product_id} // Required only if not a product
                                                    disabled={!!item.product_id} // Disabled if product is selected
                                                />
                            ...
                            <div className="flex items-center justify-end gap-4 pt-4">
                                <Link href={route('crm.offers.index')}>
